...
function addTasksByCategory(data) {
    addTasks(data.todo, false);
    addTasks(data.inProgress, false);
    addTasks(data.done, true);
}

function addTasks(data, categoryDone) {
    let counter = 0;
    while (data.length > counter) {
        ...
        if (new Date(getUpdatedTaskDate(task.dueDate)).getTime() < new Date().getTime() && !categoryDone) {
